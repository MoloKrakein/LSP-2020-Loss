    ...
    <table class="table table-dark table-hover">
      <thead>
        <tr>
          ...
          <th scope="col">KNTL</th> 
          <th scope="col"><a href="#"><button type="button" class="btn btn-primary">Tambah</button></a></th>
        </tr>
      </thead>
      <tbody>
        <?php
        include("koneksi.php");
        $tampil = mysqli_query($db, "SELECT * FROM `koki`");
        $nomer = 0;
        while ($data = mysqli_fetch_array($tampil)) {
          $nomer++
        ?>
          <tr>
            <th scope="row"><?php $nomer?></th>
            <td><?php echo $data['nama_koki']?></td>
            <td><?php echo $data['id_koki']?></td>
